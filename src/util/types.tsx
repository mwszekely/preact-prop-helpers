...
// TODO: These are only used by use*SelectionDeclarative and should be removed
...
// Helpers to build an object out of a list
...
type GenericParameters<N extends string, P extends {}, Deps extends any[]> = OmitNever<{ [Name in N as `${Name}Parameters`]: P }> & Dependencies<Deps>
type GenericReturnType<N extends string, R extends {}, Deps extends any[]> = OmitNever<{ [Name in N as `${Name}Return`]: R }> & Dependencies<Deps>
...
export interface StandardDepsPropsStable<E extends Element> { propsStable: ElementProps<E>; }
// This adds the "context" prop that parents provide/children consume
export interface StandardDepsContext<C extends {}, K extends keyof C = keyof C> { context: Pick<C, K> };
// This adds the "info" prop child hooks provide/consume
export interface StandardDepsInfo<M extends {}, K extends keyof M> { info: Pick<M, K> };

type OmitUsualSuspects<T> = Omit<T, `info` | `props${string}` | `context${string}`>
type PorR<
    Mode extends "params" | "return",
    Base extends (...args: any) => any
> = Mode extends "params" ? Parameter<Base> : ReturnType<Base>
 ...
 * 
 */
export type StandardDepsPick<
    Mode extends "params" | "return",
    Base extends (...args: any) => any,
    CategoryToModify extends keyof PorR<Mode, Base> = never,
    Action2 extends "pick" | "omit" = never,
    PropsInCategory extends keyof PorR<Mode, Base>[CategoryToModify] = never
> = OmitUsualSuspects<(
        [Action2] extends [never] ? (Pick<PorR<Mode, Base>, [CategoryToModify] extends [never] ? keyof PorR<Mode, Base> : CategoryToModify>) :
        Action2 extends "pick" ? (TargetedPick<PorR<Mode, Base>, CategoryToModify, PropsInCategory>) :
        (TargetedOmit<PorR<Mode, Base>, CategoryToModify, PropsInCategory>)
    )>
 ...
 */
export type StandardDepsOmit<
    Mode extends "params" | "return",
    Base extends (...args: any) => any,
    ExtraOmits extends Exclude<keyof PorR<Mode, Base>, "_e"> = never,
    Replacements extends any[] = []
> = OmitUsualSuspects<OmitStrong<PorR<Mode, Base>, (ExtraOmits | keyof Dependencies<Replacements>)> & Dependencies<Replacements>>


export type StandardDepsRename<D, From extends keyof D, To extends string> = { [K in From as To]: D[K] }

type PickOrOmit<Action extends "pick" | "omit", T, K extends keyof T> = Action extends "pick" ? Pick<T, K> : OmitStrong<T, K>
