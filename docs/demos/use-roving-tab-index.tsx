...
export const DemoUseRovingTabIndex = memo(() => {
    ...
    return (
        <div className="demo">
            ...
            <label># of items<input type="number" value={count} min={0} onChange={e => { e.preventDefault(); setCount(e.currentTarget.valueAsNumber) }} /></label>
            <button onClick={() => shuffle()}>Shuffle</button>
            <button onClick={() => {debugger; reverse()}}>Reverse</button>
            <label>Imperatively set the tabbable index to: <input type="number" onChange={e => { e.preventDefault(); setTabbableIndex(e.currentTarget.valueAsNumber, e, false); }} /></label>
            <label>Imperatively set the selected index to: <input type="number" onChange={e => { e.preventDefault(); setSelectedIndex(e.currentTarget.valueAsNumber); }} /> (currently {selectedIndex})</label>
            <label>Pagination window starts at: <input type="number" value={min ?? undefined} min={0} max={max ?? undefined} onChange={e => { e.preventDefault(); setMin(e.currentTarget.valueAsNumber); }} /></label>
            <label>Pagination window ends at: <input type="number" value={max ?? undefined} min={min ?? undefined} max={count} onChange={e => { e.preventDefault(); setMax(e.currentTarget.valueAsNumber); }} /></label>
            <label>Stagger delay: <input type="checkbox" checked={staggered} onChange={e => { e.preventDefault(); setStaggered(e.currentTarget.checked); }} /></label>
            <label>Selection mode:
                <label><input name="rti-demo-selection-mode" type="radio" checked={selectionMode == 'focus'} onChange={e => { e.preventDefault(); setSelectionMode("focus"); }} /> On focus</label>
                <label><input name="rti-demo-selection-mode" type="radio" checked={selectionMode == 'activation'} onChange={e => { e.preventDefault(); setSelectionMode("activation"); }} /> On activation (click, tap, Enter, Space, etc.)</label>
            </label>
...
const DemoUseRovingTabIndexChild = memo((({ index }: { index: number }) => {
    ...
    const props2 = useMergedProps<HTMLLIElement>(props, propsPress);
    ...
    return (
        <li {...props2} style={{ opacity: hideBecausePaginated ? 0.25 : 1, transform: `translateX(${hideBecauseStaggered ? "50%" : "0%"})` }}>{text}<input {...useMergedProps<HTMLInputElement>({ type: "number", tabIndex: props.tabIndex })} style={{ width: "5ch" }} /></li>
    )
}));