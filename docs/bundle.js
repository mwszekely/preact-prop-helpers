var bundle = (function (exports) {
    ...
    */
    ...
    function useListNavigation({ initialIndex, focusOnChange, collator, keyNavigation, indexMangler, indexDemangler }) {
        ...
        const navigateToFirst = A$1(() => { setTabbableIndex(0); }, []);
        const navigateToLast = A$1(() => { setTabbableIndex(managedChildren.length - 1); }, []);
        const navigateToPrev = A$1(() => { setTabbableIndex(i => indexDemangler(indexMangler(i ?? 0) - 1)); }, [indexMangler, indexDemangler]);
        const navigateToNext = A$1(() => { setTabbableIndex(i => indexDemangler(indexMangler(i ?? 0) + 1)); }, [indexMangler, indexDemangler]);
    ...
    function useGridNavigation({ focusOnChange: foc, indexMangler, indexDemangler }) {
        ...
        A$1((i) => { setCurrentRow(i ?? 0); }, []);
        const navigateToFirst = A$1(() => { setCurrentRow(0); }, []);
        const navigateToLast = A$1(() => { setCurrentRow(managedChildren.length - 1); }, []);
        const navigateToPrev = A$1(() => { setCurrentRow(i => indexDemangler(indexMangler(i ?? 0) - 1)); }, [indexMangler, indexDemangler]);
        const navigateToNext = A$1(() => { setCurrentRow(i => indexDemangler(indexMangler(i ?? 0) + 1)); }, [indexMangler, indexDemangler]);
