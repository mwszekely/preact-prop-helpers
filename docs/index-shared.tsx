    ...
    return (
        <div {...p}>
            ...
            {droppedFiles != null && <div>Files dropped: <table>
                ...
                <tbody>{droppedFiles.map(f => <tr><td>{f.name}</td>{f.data.byteLength}<td>{f.type}</td><td>{new Date(f.lastModified ?? 0).toISOString()}</td></tr>)}</tbody>
            </table></div>}
            <hr />
...
const DemoUseAsyncHandler1 = memo(() => {
    ...
    const {
        ...
        rejectCount,
        resolveCount
    } = useAsyncHandler<MouseEvent<HTMLButtonElement>, void>({
...
function DemoLabel() {
    const { propsInput, propsLabel } = useRandomDualIds<HTMLInputElement, HTMLLabelElement>({ randomIdInputParameters: { prefix: "input-", otherReferencerProp: "for" as never }, randomIdLabelParameters: { prefix: "label-", otherReferencerProp: "aria-labelledby" as never } })
    return (
        <div className="demo">
            <h2>Labels</h2>
            <input {...propsInput} />
...
const DemoGlobalHandlerChild = memo(function DemoGlobalHandlerChild({ mode, target }: { target: Window | Document, mode: "grouped" | "single" | null }) {
    ...
    useGlobalHandler(target, "click", mode == null ? null : (e: Event) => {
