import { useInsertionEffect, memo, forwardRef, createPortal } from 'preact/compat';
export { createPortal, forwardRef, memo } from 'preact/compat';
import { useCallback as useCallback$1, useRef, useLayoutEffect, useMemo, useEffect, useState as useState$1, useId, useImperativeHandle, useContext } from 'preact/hooks';
export { useContext, useDebugValue, useEffect, useId, useImperativeHandle, useLayoutEffect, useMemo, useReducer, useRef, useState as useStateBasic } from 'preact/hooks';
import { options, createElement, Fragment, cloneElement, createContext } from 'preact';
...
const useCallback = useCallback$1;
 ...
 * @returns
 */
function usePassiveState(onChange, getInitialValue, { debounceRendering: customDebounceRendering, initialization } = { debounceRendering, initialization: "delay" }) {
    initialization ??= "off";
 ...
 */
function preactAddUseInsertionEffectHook(options) {
 ...
 * @param inputs
 */
const useBeforeLayoutEffect = (thisIsPreactSoWeNeedAPolyfill ? useBeforeLayoutEffectPreact : useInsertionEffect);
...
function useLostFocusDismiss({ refElementPopupReturn: { getElement: getPopupElement, ...void3 }, refElementSourceReturn, lostFocusDismissParameters: { dismissLostFocusActive: open, onDismissLostFocus: onClose, ...void4 }, ...void1 }) {
    return useMonitoring(function useLostFocusDismiss() {
        const { getElement: getSourceElement} = (refElementSourceReturn ?? {});
        const stableOnClose = useStableGetter(onClose);
        const getOpen = useStableGetter(open);
...
const top = 0;
function parent(i) { return ((i + 1) >>> 1) - 1; }
...
function useManagedChildren(parentParameters) {
    return useMonitoring(function useManagedChildren() {
        const { managedChildrenParameters: { onChildrenMountChange, onChildrenCountChange }} = parentParameters;
        /**
 ...
 */
function useChildrenFlag({ getChildAt, forEachChild, indexFromRepositionedToOriginal, indexFromOriginalToRepositioned, initialIndex, closestFit, onClosestFit, onIndexChange, getAt, setAt, isValid }) {
    initialIndex ??= null;
 ...
 * @compositeParams
 */
function useTypeaheadNavigationChild({ info: { index, untabbable, ...void1 }, 
...
function useListNavigation({ linearNavigationParameters, typeaheadNavigationParameters, rovingTabIndexParameters, managedChildrenReturn, refElementReturn, paginatedChildrenParameters, processedIndexManglerReturn, ...void1 }) {
    return useMonitoring(function useListNavigation() {
        ...
        const { propsStable: propsStableTN, typeaheadNavigationReturn, context: contextTypeahead} = useTypeaheadNavigation({ rovingTabIndexReturn, typeaheadNavigationParameters, processedIndexManglerReturn, managedChildrenReturn });
        const { propsStable: propsStableLN, linearNavigationReturn} = useLinearNavigation({ rovingTabIndexReturn, linearNavigationParameters, paginatedChildrenParameters, processedIndexManglerReturn, managedChildrenReturn });
        return {
 ...
 * @compositeParams
 */
function useGridNavigation({ gridNavigationParameters: { onTabbableColumnChange, initiallyTabbableColumn, ...void3 }, linearNavigationParameters, managedChildrenReturn, ...listNavigationParameters }) {
    return useMonitoring(function useGridNavigation() {
        ...
        const { linearNavigationReturn, rovingTabIndexReturn, typeaheadNavigationReturn, managedChildrenParameters, context: { rovingTabIndexContext, typeaheadNavigationContext }, props} = useListNavigation({
...
refElementReturn, ...void1 }) {
    return useMonitoring(function useGridNavigationRow() {
        ...
        const { props: propsLNC, info: { getLocallyTabbable, setLocallyTabbable}, hasCurrentFocusParameters, pressParameters, rovingTabIndexChildReturn, textContentParameters} = useListNavigationChild({ info: { index, untabbable }, refElementReturn, context: contextFromParent });
        ...
        const { props: propsLN, context: contextULN, linearNavigationReturn, managedChildrenParameters, rovingTabIndexReturn, typeaheadNavigationReturn} = useListNavigation({
            managedChildrenReturn,
            refElementReturn,
 ...
 * @compositeParams
 */
function usePaginatedChildren({ managedChildrenReturn: { getChildAt, getHighestChildIndex, getLowestChildIndex }, paginatedChildrenParameters: { paginationMax: paginationMaxU, paginationMin: paginationMinU, childCount }, rovingTabIndexReturn: { getTabbableIndex, setTabbableIndex }, childrenHaveFocusReturn: { getAnyFocused }, processedIndexManglerReturn: { indexFromOriginalToRepositioned, indexFromRepositionedToOriginal } }) {
...
function usePaginatedChild({ info: { index }, context: { paginatedChildContext: { parentIsPaginated, getDefaultPaginationVisible } } }) {
    return useMonitoring(function usePaginatedChild() {
        ...
        return {
            props: useTagProps(!parentIsPaginated ? {} : { "aria-setsize": childCountIfPaginated ?? undefined, "aria-posinset": (index + 1) }),
 ...
 * @compositeParams
 */
function useRearrangeableChildren({ rearrangeableChildrenParameters: { children: childrenIn, animate, reorderedIndexProp }, processedIndexManglerParameters: { getIndex, getSortValueAt }, managedChildrenReturn: { getChildAt }, context: { processedIndexManglerContext: { mangler } } }) {
 ...
 * @compositeParams
 */
function useStaggeredChildren({ managedChildrenReturn: { getChildAt, ...void1 }, staggeredChildrenParameters: { staggered, childCount, disableIntersectionObserver, ...void2 }, processedIndexManglerReturn: { indexFromOriginalToRepositioned, indexFromRepositionedToOriginal, ...void3 }, ...void4
 ...
 * @compositeParams
 */
function useProcessedChildren({ rearrangeableChildrenParameters, paginatedChildrenParameters, staggeredChildrenParameters, context, managedChildrenParameters, processedIndexManglerParameters, ...void1 }) {
 ...
 * @compositeParams
 */
function useMultiSelection({ multiSelectionParameters: { onSelectionChange, multiSelectionAriaPropName, multiSelectionMode, ...void3 }, managedChildrenReturn: { forEachChild, getChildAt, ...void1 }, childrenHaveFocusReturn: { getAnyFocused, ...void4 }, ...void2 }) {
 ...
 * @compositeParams
 */
function useSingleSelectionChild({ singleSelectionChildParameters: { singleSelectionDisabled, ...void5 }, context: { singleSelectionContext: { getSingleSelectedIndex, onSingleSelectedIndexChange, singleSelectionAriaPropName: ariaPropName, singleSelectionMode, ...void1 }, ...void2 }, info: { index: indexUntyped, untabbable, ...void3 }, ...void4 }) {
 ...
 */
function useSingleSelectionDeclarative({ singleSelectionReturn: { changeSingleSelectedIndex }, singleSelectionDeclarativeParameters: { singleSelectedIndex: ssiu, onSingleSelectedIndexChange } }) {
 ...
 * @compositeParams
 */
function useSelection({ managedChildrenReturn, multiSelectionParameters, childrenHaveFocusReturn, rovingTabIndexReturn, singleSelectionParameters, processedIndexManglerReturn }) {
    const { childrenHaveFocusParameters: { onCompositeFocusChange: ocfc1}, context: contextSS, singleSelectionReturn} = useSingleSelection({ managedChildrenReturn, rovingTabIndexReturn, singleSelectionParameters, processedIndexManglerReturn });
    const { childrenHaveFocusParameters: { onCompositeFocusChange: ocfc2}, context: contextMS, multiSelectionReturn, propsStable} = useMultiSelection({ managedChildrenReturn, multiSelectionParameters, childrenHaveFocusReturn });
    return {
        propsStable,
...
function useSelectionChild({ context, info: { index, untabbable, ...void2 }, singleSelectionChildParameters, multiSelectionChildParameters, ...void3 }) {
    const { props: p1, hasCurrentFocusParameters: { onCurrentFocusedInnerChanged: ocfic1 }, info: { getSingleSelected, setLocalSingleSelected, singleSelected}, singleSelectionChildReturn, } = useSingleSelectionChild({ context, info: { index, untabbable }, singleSelectionChildParameters });
    const { props: p2, hasCurrentFocusParameters: { onCurrentFocusedInnerChanged: ocfic2 }, multiSelectionChildReturn, info: { getMultiSelected, setSelectedFromParent, getMultiSelectionDisabled}} = useMultiSelectionChild({ context, info: { index }, multiSelectionChildParameters });
    return {
...
function useRefElement(args) {
    ...
    const [getElement, setElement] = usePassiveState(handler, returnNull, { debounceRendering: runImmediately, initialization: "delay" });
...
function useActiveElement({ activeElementParameters: { onActiveElementChange, onLastActiveElementChange, onWindowFocusedChange, getDocument } }) {
    return useMonitoring(function useActiveElement() {
        ...
        const [getActiveElement, setActiveElement] = usePassiveState(onActiveElementChange, returnNull, { debounceRendering: runImmediately, initialization: "off" });
        const [getLastActiveElement, setLastActiveElement] = usePassiveState(onLastActiveElementChange, returnNull, { debounceRendering: runImmediately, initialization: "off" });
        const [getWindowFocused, setWindowFocused] = usePassiveState(onWindowFocusedChange, returnTrue, { debounceRendering: runImmediately, initialization: "off" });
...
function useDismiss({ dismissParameters: { dismissActive, onDismiss, ...void3 }, backdropDismissParameters: { dismissBackdropActive, onDismissBackdrop, ...void6 }, lostFocusDismissParameters: { dismissLostFocusActive, onDismissLostFocus, ...void7 }, escapeDismissParameters: { dismissEscapeActive, onDismissEscape, parentDepth, ...void2 }, activeElementParameters: { getDocument, onActiveElementChange, onLastActiveElementChange: olaec1, onWindowFocusedChange, ...void5 }, ...void4 }) {
    return useMonitoring(function useDismiss() {
        ...
        const { activeElementParameters: { onLastActiveElementChange: olaec2} } = useLostFocusDismiss({
...
function useGridNavigationSelection({ gridNavigationParameters, linearNavigationParameters, rovingTabIndexParameters, managedChildrenReturn, typeaheadNavigationParameters, singleSelectionParameters, multiSelectionParameters, refElementReturn, paginatedChildrenParameters, processedIndexManglerReturn, childrenHaveFocusReturn, ...void2 }) {
    return useMonitoring(function useGridNavigationSelection() {
        const { props, context: { gridNavigationRowContext, rovingTabIndexContext, typeaheadNavigationContext }, linearNavigationReturn, rovingTabIndexReturn, managedChildrenParameters, typeaheadNavigationReturn, } = useGridNavigation({
...
function useGridNavigationSelectionRow({ info: mcp1, linearNavigationParameters, managedChildrenReturn, refElementReturn, rovingTabIndexParameters, typeaheadNavigationParameters, context, singleSelectionChildParameters, multiSelectionChildParameters, ...void1 }) {
    return useMonitoring(function useGridNavigationSelectionRow() {
        ...
        const { hasCurrentFocusParameters: { onCurrentFocusedInnerChanged: ocfic2}, info: { focusSelf, getLocallyTabbable, setLocallyTabbable}, props: propsGridNavigation, linearNavigationReturn, managedChildrenParameters, pressParameters: { excludeSpace}, rovingTabIndexChildReturn, rovingTabIndexReturn, textContentParameters, typeaheadNavigationReturn, context: contextGridNavigation} = useGridNavigationRow({ context, linearNavigationParameters, info: mcp1, managedChildrenReturn, refElementReturn, rovingTabIndexParameters, typeaheadNavigationParameters });
        let ret = {
...
function useListNavigationSelectionChild({ info: { index, untabbable, ...void2 }, context, refElementReturn, singleSelectionChildParameters, multiSelectionChildParameters, ...void1 }) {
    return useMonitoring(function useListNavigationSelectionChild() {
        ...
        const { hasCurrentFocusParameters: { onCurrentFocusedInnerChanged: ocfic1}, pressParameters: { excludeSpace }, rovingTabIndexChildReturn, textContentParameters, props: propsLN, info: infoLN} = useListNavigationChild({
            info: { index, untabbable },
            context,
            refElementReturn,
        });
...
function useChildrenHaveFocus(args) {
    return useMonitoring(function useChildrenHaveFocus() {
        ...
        const [getAnyFocused, setAnyFocused] = usePassiveState(onCompositeFocusChange, returnFalse, { debounceRendering: runImmediately, initialization: "delay" });
        ...
        }), returnZero, { debounceRendering: setTimeout, initialization: "delay" }); // setTimeout is used for the debounce to be somewhat generous with timing, and to guard against the default being able to be runImmediately...
        return {
...
function useHasCurrentFocus(args) {
    return useMonitoring(function useHasCurrentFocus() {
        ...
        const [getFocused, setFocused] = usePassiveState(onCurrentFocusedChanged, returnFalse, { debounceRendering: runImmediately, initialization: "off" });
        const [getFocusedInner, setFocusedInner] = usePassiveState(onCurrentFocusedInnerChanged, returnFalse, { debounceRendering: runImmediately, initialization: "off" });
...
function useCompleteGridNavigation({ gridNavigationParameters, linearNavigationParameters, rovingTabIndexParameters, singleSelectionParameters, multiSelectionParameters, typeaheadNavigationParameters, paginatedChildrenParameters, refElementParameters, gridNavigationCompleteParameters: { getSortColumn, getSortValueAt: gsva }, processedIndexManglerParameters: { compare, getIndex }, ...void1 }) {
    return useMonitoring(function useCompleteGridNavigation() {
        ...
        const { props, context: { gridNavigationRowContext, rovingTabIndexContext, singleSelectionContext, multiSelectionContext, typeaheadNavigationContext }, childrenHaveFocusParameters, managedChildrenParameters, rovingTabIndexReturn, ...restGridNavReturns } = useGridNavigationSelection({
            gridNavigationParameters,
            singleSelectionParameters,
            multiSelectionParameters,
            paginatedChildrenParameters,
...
function useCompleteGridNavigationRow({ info: { index, untabbable, ...customUserInfo }, context: contextIncomingForRowAsChildOfTable, textContentParameters: { getText, onTextContentChange: otcc1 }, linearNavigationParameters, rovingTabIndexParameters, typeaheadNavigationParameters, hasCurrentFocusParameters: { onCurrentFocusedChanged: ocfc1, onCurrentFocusedInnerChanged: ocfic3, ...void5 }, singleSelectionChildParameters, multiSelectionChildParameters, refElementParameters, ...void1 }) {
    return useMonitoring(function useCompleteGridNavigationRow() {
        ...
        // Due to the declarations needing to come before the actual definitions,
        // we need to define these wrapper functions.
        const getHighestChildCellIndex = useCallback(() => managedChildrenReturn.getHighestChildIndex(), []);
        const getLowestChildCellIndex = useCallback(() => managedChildrenReturn.getLowestChildIndex(), []);
        const getChildCellAt = useCallback((i) => managedChildrenReturn.getChildAt(i), []);
        const forEachChildCell = useCallback((f) => managedChildrenReturn.forEachChild(f), []);
        const isCellValidForNavigation = useCallback((i) => {
...
function useCompleteGridNavigationCell({ gridNavigationCellParameters, context, textContentParameters: { getText, onTextContentChange: otcc1, ...void4 }, info: { focusSelf, index, untabbable, ...customUserInfo }, ...void1 }) {
    return useMonitoring(function useCompleteGridNavigationCell() {
        const { refElementReturn, propsStable } = useRefElement({ refElementParameters: {} });
        const { hasCurrentFocusParameters, rovingTabIndexChildReturn, textContentParameters: { onTextContentChange: otcc2 }, pressParameters: { excludeSpace: es1 }, props: propsRti, info: info2} = useGridNavigationSelectionCell({
 ...
 * @compositeParams
 */
function useCompleteListNavigation({ linearNavigationParameters, typeaheadNavigationParameters, rovingTabIndexParameters, singleSelectionParameters, multiSelectionParameters, paginatedChildrenParameters, refElementParameters, processedIndexManglerParameters, ...void1 }) {
...
textContentParameters: { getText, onTextContentChange: otcc1, ...void10 }, refElementParameters, hasCurrentFocusParameters: { onCurrentFocusedChanged, onCurrentFocusedInnerChanged: ocfic3, ...void7 }, singleSelectionChildParameters, multiSelectionChildParameters, context: { managedChildContext, rovingTabIndexContext, singleSelectionContext, multiSelectionContext, typeaheadNavigationContext, childrenHaveFocusChildContext, processedChildrenContext, processedIndexManglerContext, listNavigationCompleteContext, ...void5 }, ...void1 }) {
    return useMonitoring(function useCompleteListNavigationChild() {
        const { refElementReturn, propsStable} = useRefElement({ refElementParameters });
        const { hasCurrentFocusParameters: { onCurrentFocusedInnerChanged: ocfic1}, pressParameters: { excludeSpace}, selectionChildReturn, textContentParameters: { onTextContentChange: otcc2}, singleSelectionChildReturn, multiSelectionChildReturn, info: infoFromListNav, rovingTabIndexChildReturn, propsChild, propsTabbable} = useListNavigationSelectionChild({
            info: { index, untabbable },
...
function useHasLastFocus(args) {
    return useMonitoring(function useHasLastFocus() {
        const { refElementReturn: { getElement }, activeElementParameters: { onLastActiveElementChange, ...activeElementParameters }, hasLastFocusParameters: { onLastFocusedChanged, onLastFocusedInnerChanged} } = args;
...
function useAsyncEffect(effect, inputs, options) {
    const { syncHandler, ...rest } = useAsync(effect, { capture: null, debounce: null, throttle: null });
 ...
 * @returns
 */
function usePropsOnChildren(children, props, Tag = 'span') {
...
function useSearchParamState({ key: paramKey, defaultReason, stringToValue, initialValue, onValueChange, valueToString }) {
    ...
    }), { debounceRendering: runImmediately, initialization: "delay" });
